...
public class CommentControllerTest {
  ...
  @InjectMocks private CommentController commentControllerMock;
  ...
  private List<Comment> createCommentList(Long vehicleId, int number) {
    ...
    try {
      for (int i = 1; i <= number; i++) {
        ...
        comment =
            new Comment((long) i, vehicleId, "test_" + i, "user_" + i, timestamp, (i - 1) % 5 + 1);
  ...
  public void add() throws Exception {
    ...
    String getAllForVehicleTest =
        mockMvc
            .perform(get("/comments/all/1"))
            ...
            .andExpect(jsonPath("$[3].rating", is(4)))
            .andReturn()
            .getResponse()
            .getContentAsString();
  ...
  public void shouldShowPagedCommentsList() throws Exception {
    ...
    Mockito.when(commentService.getCommentsForVehicle((long) 1, new PageRequest(0, 10)))
        .thenReturn(page);

    String getCommentsForVehicle =
        mockMvc
            .perform(get("/comments/1?page=0&number=10"))
            .andExpect(status().isOk())
            ...
            .andExpect(jsonPath("$.content[0].rating", is(1)))
            ...
            .andExpect(jsonPath("$.content[1].rating", is(2)))
            ...
            .andExpect(jsonPath("$.content[3].rating", is(4)))
    ...
    Mockito.when(commentService.getCommentsForVehicle((long) 2, new PageRequest(3, 5)))
        .thenReturn(page2);

    String getCommentsForVehicle2 =
        mockMvc
            .perform(get("/comments/2?page=3&number=5"))
            .andExpect(status().isOk())
    ...
    Mockito.verify(commentService, times(1))
        .getCommentsForVehicle((long) 1, new PageRequest(0, 10));
