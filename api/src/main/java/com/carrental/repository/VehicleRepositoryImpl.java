...
public class VehicleRepositoryImpl implements VehicleRepositoryCustom {
  ...
  public Vehicle getVehicleUsingId(Long id) {
    ...
    try {
      vehicle =
          (Vehicle)
              entityManager
                  .createQuery("SELECT v FROM Vehicle v WHERE v.id = :vehicle_id")
                  .setParameter("vehicle_id", id)
                  .getSingleResult();
            ...
            entityManager
                .createQuery(
                    "SELECT v FROM Vehicle v JOIN VehicleParameters vp ON (v.id=vp.vehicleID) JOIN Location l ON (l.id=v.locationId) WHERE "
                        ...
                        + "(:color IS NULL OR vp.color=:color) "
                        + " ORDER BY v.id ")
                ...
                .setParameter("color", vehicleFilter.getColor())
  ...
  public List<String> getBrandList() {
    return (List<String>)
        entityManager.createQuery("SELECT distinct v.brand FROM Vehicle v").getResultList();
  }
  ...
  public List<String> getModelListForBrand(String brand) {
    ...
    return (List<String>)
        entityManager
            .createQuery("SELECT distinct v.model FROM Vehicle v WHERE v.brand=:br")
            .setParameter("br", formatedBrand)
            .getResultList();
  }
  ...
  public List<String> getCityList() {
    return (List<String>)
        entityManager.createQuery("SELECT l.city FROM Location l").getResultList();
  }
  ...
  public List<String> getColorList() {
    return (List<String>)
        entityManager
            .createQuery(
                "SELECT distinct vp.color FROM Vehicle v JOIN VehicleParameters vp ON (v.id=vp.vehicleID)")
            .getResultList();
  }
        ...
        entityManager
            .createQuery(
                "UPDATE Vehicle v SET "
                    ...
                    + "v.dailyFee=COALESCE(:dailyFee,v.dailyFee), "
                    ...
                    + "v.bestOffer=COALESCE(:bestOffer,v.bestOffer) "
                    + "WHERE v.id=:id")
            .setParameter("brand", vehicleAddDto.getBrand())
            .setParameter("model", vehicleAddDto.getModel())
            ...
            .setParameter("bestOffer", vehicleAddDto.getBestOffer())
            .setParameter("id", vehicleAddDto.getId())
            .executeUpdate();
    ...
    entityManager
        .createQuery(
            "UPDATE VehicleParameters vp SET "
                ...
                + "vp.color=COALESCE(:color,vp.color), "
                + "vp.metallic=COALESCE(:metallic,vp.metallic), "
        ...
        .setParameter("color", vehicleAddDto.getColor())
        .setParameter("metallic", vehicleAddDto.getMetallic())
        .setParameter("description", vehicleAddDto.getDescription())
        .setParameter("productionYear", vehicleAddDto.getProductionYear())
        .setParameter("photoName", vehicleAddDto.getFileName())
        .setParameter("id", vehicleAddDto.getId())
        .executeUpdate();
  ...
  public void addVehicle(VehicleAddDTO vehicleAddDto) {
    Vehicle vehicle =
        new Vehicle(
            ...
            vehicleAddDto.getBrand(),
            vehicleAddDto.getModel(),
            vehicleAddDto.getDailyFee(),
            vehicleAddDto.getLocation(),
            vehicleAddDto.getVehicleStatus(),
            (vehicleAddDto.getBestOffer() != 0 ? true : false));
    ...
    vehicleParameters =
        new VehicleParameters(
            ...
            vehicleAddDto.getGearbox(),
            vehicleAddDto.getFrontWheelDrive(),
            vehicleAddDto.getDoorsNumber(),
            vehicleAddDto.getSeatsNumber(),
            vehicleAddDto.getColor(),
            vehicleAddDto.getMetallic(),
            vehicleAddDto.getFileName(),
