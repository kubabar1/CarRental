...
export class PaginationCarList extends React.Component {
	...
	create_items = () => {
		...
		for(var i = start; i < max; i++){
			objs.push(<li key={"page_"+i} name={"page_"+i} id={"page_"+i} className="page-item"><Link onClick={this.updateVehicleList.bind(this,i)} to={"/CarRental/listcar?page="+i}  className="page-link">{(i+1)}</Link></li>);
		}
	...
	renderContent = (previous,next) => {
		...
		const url = this.props.filterWrapper ? "/CarRental/listcar" : "/CarRental/listcar/searchresult";

		return(
			<div className="text-center my-5">
				<ul className="pagination text-center">
					<li key={"page_first"} name={"page_first"} id={"page_first"} className="page-item"><Link onClick={()=>this.updateVehicleList(0)} className="page-link" to="/CarRental/listcar?page=0" >First</Link></li>
					...
					<li key={"page_last"} name={"page_last"} id={"page_last"} className="page-item"><Link onClick={()=>this.updateVehicleList(totalPages-1)} to={"/CarRental/listcar?page="+(totalPages-1)} className="page-link">Last</Link></li>
				</ul>
			</div>
		);
	}
