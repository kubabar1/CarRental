...
export class ShowCarFeatures extends React.Component {
        ...
        return (
            <div className="p-3 table-responsive">
                <table className="table table-striped table-hover">
                    ...
                    <tbody>{featuresList ? featuresList.map(this.renderRow) : ''}</tbody>
                </table>
            </div>
        );
    };
    ...
    renderRow = (equipment) => {
        ...
        return (
            <tr key={eqCode}>
                <td>
                    <button onClick={() => this.deleteEquipment(eqCode)} className={'btn btn-danger'} type="button">
                        Delete
                    </button>
                </td>
                <td>{eqCode}</td>
    ...
    refreshLists = (car_id) => {
        fetch('http://localhost:8080/CarRental/carlist/' + car_id)
            ...
            .then((data) => {
                this.setState({ featuresList: data.equipmentList });
            })
    ...
    renderFeatureOption = (feature) => {
        return (
            <option name={'feature_' + feature.equipmentCode} key={feature.equipmentCode} value={feature.equipmentCode}>
                {feature.description}
            </option>
        );
    };
                ...
                <div className="row my-3">
                    ...
                    <div className="mt-3 ml-3">
                        <select
                            ...
                            value={selectedEquipment == null ? 'null' : selectedEquipment}
                            onChange={this.handleOptionChange}
                        >
                            <option key="dispabled_feature" name="dispabled_feature" value="null" disabled="disabled">
                                Choose equipment
                            </option>
